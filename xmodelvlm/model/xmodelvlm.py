...
from xmodelvlm.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, \
    DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from xmodelvlm.model.vision_encoder import build_vision_tower
from xmodelvlm.model.vision_projector import build_vision_projector


class XmodelVLMMetaModel:

    def __init__(self, config):
        super(XmodelVLMMetaModel, self).__init__(config)
...
class XmodelVLMMetaForCausalLM(ABC):
    ...
    def encode_cross_images(self, images, text_features):
        image_features = self.get_model().get_vision_tower()(images)
        image_features = self.get_model().mm_projector(image_features, text_features)
        return image_features
...
def load_pretrained_model(model_path, load_8bit=False, load_4bit=False, device_map="auto", device="cuda"):
    from xmodelvlm.model.xmodel_lm import XmodelLMForCausalLM
    ...
    model = XmodelLMForCausalLM.from_pretrained(model_path, trust_remote_code=True, low_cpu_mem_usage=True, **kwargs)
